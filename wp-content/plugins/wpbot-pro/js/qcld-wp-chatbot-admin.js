...
$(document).on('change','#show_intent_navigation_notification',function (e) {
    ...
    jQuery('body').on('change','#wp_chatbot_exit_intent_email_ckeckbox',function(){
      if($(this).is(':checked')){
          $('#wp_chatbot_exit_intent_email').val(1)
      }else{
          $('#wp_chatbot_exit_intent_email').val(0)
      }     
    })
    jQuery('body').on('change','#wp_chatbot_auto_open_email_checked',function(){
        if($(this).is(':checked')){
            $('#wp_chatbot_auto_open_email').val(1)
        }else{
            $('#wp_chatbot_auto_open_email').val(0)
        }     
    })
    jQuery('body').on('change','#wp_chatbot_scroll_open_email_checked',function(){
        if(jQuery(this).is(':checked')){
            jQuery('#wp_chatbot_scroll_open_email').val(1)
        }else{
            jQuery('#wp_chatbot_scroll_open_email').val(0)
        }     
    })
...
(function($) {
    ...
    if(flieElementOpenAi){
        ...
        $(".error-message").on('click','.alert .close',function(){
            $(".alert").alert('close')
        })
      
        $("#openaiFTList").on('click','.remove_ft_model_openai',function(){
            $(this).hide();
        ...
        $("#qcld-ft-modal").on('click','.create_ft_model', function(){
           
            var ft_suffix = $('#qcld_openai_ft_suffix').val();
            var file_id = $('#qcld_openai_ft_fileid').val();
            var ft_engines = $("[id*='qcld_openai_ft_engines'] :selected").val();
            console.log(ft_engines)
            $.ajax({
                url: ajax_object.ajax_url,
                type: 'POST',
                dataType: "JSON",
                data:  {
                    action : 'openai_ft_model_create',
                    file_id: file_id,
                    ft_suffix: ft_suffix,
                    ft_engines: ft_engines,
                },
                success: function (response) {
                    location.reload();
                },  
    ...
    if(settingsOpenAi){
        ...
        $('.qcl-openai').on('click', '#save_setting', function(){
                ...
                var qcld_openai_suffix = $("input[name='qcld_openai_suffix']" ).val();
                var qcld_openai_custom_model = $("input[name='qcld_openai_custom_model']" ).val();
                var file_id = $("input[name='file_id']" ).val();
                ...
                $.ajax({
                    url: ajax_object.ajax_url,
                    type:'POST',
                    data:    ({action  : 'openai_settings_option',api_key: api_key,openai_engines:openai_engines,qcld_openai_prompt: qcld_openai_prompt,qcld_openai_suffix:qcld_openai_suffix,max_tokens:max_tokens,file_id:file_id,temperature:temperature,presence_penalty:presence_penalty,frequency_penalty:frequency_penalty,qcld_openai_prompt_custom: qcld_openai_prompt_custom,qcld_openai_custom_model:qcld_openai_custom_model,ai_enabled:is_ai_enabled,ai_only_mode: is_ai_only_mode}),
                    success: function(data){
                        $('#result').html(data);
                        location.reload();
                    }
