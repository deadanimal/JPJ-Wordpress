...
function qc_wpbo_search_response(){
	...
	if(empty($response_result)){
		...
		$results = $wpdb->get_results("SELECT `query`, `response`, `custom`, `trigger_intent`, MATCH($qfields) AGAINST('".$keyword."' IN NATURAL LANGUAGE MODE) as score FROM $table WHERE MATCH($qfields) AGAINST('".$keyword."' IN NATURAL LANGUAGE MODE) and lang='".$language."' and `hidden` = 0 order by score desc limit 15");
		...
		if(!empty($results)){
			foreach($results as $result){
				if($result->score >= $weight){
					...
					$response_result[] = array( 'query'=>$result->query, 'response'=> (get_option('qc_bot_str_allow_shortcode') && get_option('qc_bot_str_allow_shortcode')==1?apply_filters('the_content', $result->response):$result->response ), 'followup'=>(get_option('qc_bot_str_allow_shortcode') && get_option('qc_bot_str_allow_shortcode')==1?apply_filters('the_content', $result->custom):$result->custom ), 'trigger_intent' => $result->trigger_intent, 'score'=>$result->score, 'users_answer' => $users_answer);
