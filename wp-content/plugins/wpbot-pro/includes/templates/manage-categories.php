...
<div class="qcwrap">
    <div class="wp-chatbot-wrap">
		...
		<div class="wpbot_addons_section container">
			...
			<a href="<?php echo add_query_arg( 'opt', 'add', admin_url('admin.php?page=simple-text-response&action=manage-categories') ); ?>" class="page-title-action button">Add New Category</a>
		
			<div class="wpbot_single_addon_wrapper2">
			
				<div class="dfentity_wp_table">
					<h2>Category List</h2>
					<a href="<?php echo add_query_arg( 'id', '0', admin_url('admin.php?page=simple-text-response&action=manage-categories&opt=delete') ); ?>"> Delete all</a>
					<hr>
					<?php if(!empty($data)): ?>
					<table class="qcdf_table">
						...
						<tr>
							...
							<td>
								<?php 
									$parent = $wpdb->get_row("select * from $table where 1 and id = '". $result->custom ."'");
									echo ( ! empty( $parent ) ? esc_html( $parent->name ) : '' );
								?>
							</td>
							<td>
							<a href="<?php echo add_query_arg( 'id', $result->id, admin_url('admin.php?page=simple-text-response&action=manage-categories&opt=edit') ); ?>" class="page-title-action">Edit</a>
							 | 
							<a href="<?php echo add_query_arg( 'id', $result->id, admin_url('admin.php?page=simple-text-response&action=manage-categories&opt=delete') ); ?>" class="page-title-action">Delete</a>
