...
final class Qcld_str_pro{
						...
						if ( $valid && function_exists('is_user_logged_in') && is_user_logged_in() ) 
						{
							...
							if( $tmpName != "" )
							{
								...
								foreach($baseData as $row){
									...
									//var_dump( );wp_die();
									$data = array('query' => sanitize_text_field($query), 'keyword' => sanitize_text_field($keyword), 'response'=> $response, 'intent'=> sanitize_text_field($intent), 'category'=> sanitize_text_field($row[3]), 'lang' => sanitize_text_field( $lang ) );
	...
	public function stredit(){
		...
		$users_answer = isset($data->users_answer) ? maybe_unserialize( $data->users_answer ) : 0;
				...
				<tbody>
					...
					<tr valign="top">
						...
						<td>
							...
							<br><i>Please select categories. You can select multiple category by press CTRL.</i>
						</td>
					</tr>

					<tr valign="top">
						<th scope="row">Hide this STR from Search</th>
						<td>
					...
					<tr valign="top">
						<th scope="row">Intent Name (to trigger from another Intent or Start Menu)</th>
						<td>	
					...
					<tr valign="top" style="border-bottom: 1px solid #ddd;">
						<th scope="row">Follow up Message</th>
						<td>	
							<?php 
							...
							?>
							<br><i>Optional. Follow up message will show after the response.</i>
					...
					<tr valign="top" style="border-bottom: 1px solid #ddd;">
						<th scope="row">Match User's Answer to the Follow Up Message</th>
						<td>	
							<div class="users_answer_section" >
								<div id="users_answer_container">
									<?php if ( empty( $users_answer['answer'] ) ) { ?>
									<div class="users_answer_item">
										<input type="text" name="users_answer[answer][]" placeholder="Enter Keywords to Match against the User' Answer. Separate keywords with Comma or #" />
											...
											<?php 
												foreach($allIntents as $key1 => $value){
													?>
														...
														<?php foreach($value as $val){ ?>
															<option value="<?php echo $val; ?>" <?php echo ($hasEdit && $users_answer['feedback'][$key] == $val ? 'selected="selected"' : '' ); ?> ><?php echo $val; ?></option>
														<?php } ?>
														</optgroup>
													<?php
												}
					...
					<tr valign="top" style="border-bottom: 1px solid #ddd;">
						<th scope="row">Follow Up Message If User Answer Does not Match with any keywords above</th>
						<td>	
