	...
	* get image data by id
	...
	private function getFieldValue($item, $paramName, $source, $dataItem, $param){
		...
		if(is_array($source)){
			...
			if(!empty($textBefore))
				$item[$paramName] = $textBefore.$item[$paramName];

			if(!empty($textAfter))
				$item[$paramName] = $item[$paramName].$textAfter;
		...
		switch($source){
			case "text_before":
				...
				$textBeforeKey = $this->nameParam."_text_before_{$paramName}";
			...
			case "text_after":
				...
				$textBeforeKey = $this->nameParam."_text_after_{$paramName}";
